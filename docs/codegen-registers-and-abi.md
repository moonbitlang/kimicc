...
mixed integer/SSE, and overflow-stack subset.
...
read-modify-write updates. Switch dispatch emits explicit case comparisons,
uses the promoted control expression type, preserves default and fallthrough
behavior, and compares 32-bit integer switches through `eax`. Vector,
x87/complex, vector varargs, direct ELF object emission, and Linux JIT loading
are still future work. See
