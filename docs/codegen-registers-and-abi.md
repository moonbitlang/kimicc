...
`vswprintf`, `vfprintf`, `vdprintf`, `vasprintf`, `vsscanf`, `vswscanf`, and
`vfscanf`, and also passes a `va_copy`-copied cursor back to `vsnprintf`,
which covers real v-function interop paths while leaving broader libc
