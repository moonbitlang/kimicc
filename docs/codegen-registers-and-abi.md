...
`vprintf`, `vswprintf`, `vfprintf`, `vfwprintf`, `vdprintf`, `vasprintf`,
`vsscanf`, `vswscanf`, `vfscanf`, and `vfwscanf`, and also passes a
`va_copy`-copied cursor back to `vsnprintf`, which covers real v-function
interop paths while leaving broader libc `va_list` compatibility open.
Small scalar-field
