...
| Open | SQLite `veryquick.test` post-`avtrans` frontier | `./testfixture /tmp/kimicc_sqlite_src_3049001/sqlite-src-3049001/test/veryquick.test` from an isolated temporary working directory after relinking `/tmp/kimicc_sqlite3_testfixture.o` | The current broad run exits with SIGSEGV (`rc=139`) after reaching `func.test`. Earlier visible mismatches include `decimal-6100` (`ieee754(1,-1074)` expected, `ieee754(1,4294966222)` observed) and several `fts3corrupt4` malformed-database checks that unexpectedly succeed after Tcl reports `bad option "decode": must be format or scan`. Next step is to run these scripts standalone against kimicc and clang fixtures, then minimize the first compiler-owned mismatch before chasing the later crash. |
