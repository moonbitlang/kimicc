...
| Open | QuickJS reduced embed smoke after switch dispatch fix | `/tmp/kimicc_quickjs_smoke/run_emscripten_noatomics/embed_smoke`, built from QuickJS 2025-09-13 with atomics/stack-check disabled and support files compiled by clang | The reduced smoke now preprocesses, compiles `quickjs.c` with kimicc, assembles, and links. It no longer crashes in `JS_NewRuntime2` and no longer aborts in `__JS_FreeValueRT`. The next runtime frontier is an `EXC_BAD_ACCESS` in `js_clone_shape`: `JS_DefineAutoInitProperty()` receives a `JSValue` whose object pointer resolves to a stack address, pointing at small-struct-by-value argument ABI handling for QuickJS's 16-byte `JSValue`. Normal macOS QuickJS preprocessing also still leaves compiler builtins such as fortified `snprintf`, C11 atomics, and `__builtin_frame_address`, so full unmodified QuickJS needs either builtin lowering or a sanitized preprocessor profile. |
