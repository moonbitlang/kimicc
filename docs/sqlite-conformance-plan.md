...
| Open | SQLite `veryquick.test` post-`func` frontier | `./testfixture /tmp/kimicc_sqlite_src_3049001/sqlite-src-3049001/test/veryquick.test` from an isolated temporary working directory after relinking `/tmp/kimicc_sqlite3_testfixture.o` | The current broad run completes with `rc=1`: 105 errors out of 392299 tests. Known harness/platform-noise clusters include Tcl `binary decode` failures (`fts5contentless3`, `fts3corrupt4`) and `Inf` versus `inf` casing. The next step is to run the remaining clusters standalone against kimicc and clang fixtures, starting with the likely compiler-owned `func7` math-function failures, then `incrcorrupt`, `types3`, and corruption-script clusters. |
