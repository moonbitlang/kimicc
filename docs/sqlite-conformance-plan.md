...
| Open | SQLite RTree coordinate float/double handling | `./testfixture /tmp/kimicc_sqlite_src_3049001/sqlite-src-3049001/ext/rtree/rtree7.test`; same cluster starts in `rtree2.test` and `rtree9.test` | The next `veryquick.test` run reaches RTree. `rtree2.test`, `rtree7.test`, and `rtree9.test` fail under kimicc and pass under clang. The clearest symptom is RTree coordinates such as `1.0`, `2.0`, `40.0`, and `60.0` returning as tiny doubles like `5.26354424712089e-315`, which is consistent with 32-bit float payload bits being interpreted as 64-bit doubles. Next step is to minimize the RTree read/write path and determine whether this is a float return, float-to-double argument conversion, or struct/union storage issue. |
