...
| Open | QuickJS reduced embed smoke cleanup crash | `/tmp/kimicc_quickjs_smoke/run_emscripten_noatomics/embed_smoke`, built from QuickJS 2025-09-13 with atomics/stack-check disabled and support files compiled by clang | The smoke now preprocesses, compiles, assembles, links, evaluates `let x = 1 + 2; x`, and prints `3`. It then crashes during `JS_FreeRuntime` in `gc_decref_child`: `mark_children()` sees a GETSET property with getter/setter pointers both equal to `0x000c010000000002`. Next work should minimize whether this is property payload corruption, shape/property flag handling, or a remaining aggregate/union ABI bug. Normal macOS QuickJS preprocessing also still leaves compiler builtins such as fortified `snprintf`, C11 atomics, and `__builtin_frame_address`, so full unmodified QuickJS needs either builtin lowering or a sanitized preprocessor profile. |
