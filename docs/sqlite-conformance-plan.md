...
| Open | SQLite `veryquick.test` post-`decimal` frontier | `./testfixture /tmp/kimicc_sqlite_src_3049001/sqlite-src-3049001/test/veryquick.test` from an isolated temporary working directory after relinking `/tmp/kimicc_sqlite3_testfixture.o` | The current standalone compiler-owned frontier is `func.test`, which still exits with SIGSEGV (`rc=139`) after `func-22.4`. `fts3corrupt4.test` also reports failures, but the clang-linked fixture shows the same Tcl `bad option "decode": must be format or scan` behavior, so that remains classified as harness/Tcl capability noise until the `func.test` crash is minimized. |
