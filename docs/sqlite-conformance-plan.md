...
| Passed | tree-sitter C library smoke | `KIMICC_EXTERNAL_TESTBED=tree-sitter moon test test/e2e/external_testbeds_test.mbt --target native --filter 'external tree-sitter C library smoke compiles and runs when enabled'` | The opt-in MoonBit e2e test rebuilds tree-sitter's C runtime files from `/tmp/kimicc_tree_sitter/lib/src` plus the generated mini parser from `/tmp/kimicc_tree_sitter/src/parser.c` through kimicc, links a smoke driver, parses `alpha beta gamma`, checks the printed tree for `source_file` and `word`, and exits with status 42. Routine tests skip this unless `KIMICC_EXTERNAL_TESTBED=tree-sitter` or `all` is set. |
| Passed | ocamlyacc calc grammar differential | `KIMICC_EXTERNAL_TESTBED=ocamlyacc moon test test/e2e/external_testbeds_test.mbt --target native --filter 'external ocamlyacc calc grammar matches clang when enabled'` | The opt-in MoonBit e2e test rebuilds OCaml's pure-C `yacc/` parser generator from `/tmp/kimicc_ocaml` through kimicc, builds a clang baseline from the same sources, runs both binaries on `calc_parser.mly`, and byte-compares the generated `calc_parser.ml` and `calc_parser.mli`. Routine tests skip this unless `KIMICC_EXTERNAL_TESTBED=ocamlyacc` or `all` is set. |
...
| Open | SQLite `full` permutation residual platform/Tcl expectation failures | `KIMICC_SQLITE_CONFORMANCE=full moon test test/e2e/sqlite_test.mbt --target native --filter 'sqlite public full residual failures match clang when enabled'`; fresh kimicc output at `/tmp/kimicc_sqlite_public_full_kimicc_279101241_180985381.out`; fresh clang output at `/tmp/kimicc_sqlite_public_full_clang_279101241_-1052129227.out` | The public `full` permutation completes and the MoonBit harness verifies that kimicc and clang have the same non-zero exit code plus the same residual script set: 26 errors out of 1016630 tests across `json101.test`, `func.test`, `types3.test`, `literal.test`, `json501.test`, and `loadext.test`. The first five are the same clang-matching `veryquick` residual groups. The extra `loadext.test` failures (`loadext-2.1`, `loadext-2.2`) also match clang exactly and are Darwin dynamic-loader message expectation drift, not a kimicc-only behavior difference. |
