...
| Open | SQLite `veryquick.test` post-`func7` frontier | `./testfixture /tmp/kimicc_sqlite_src_3049001/sqlite-src-3049001/test/veryquick.test` after relinking `/tmp/kimicc_sqlite3_testfixture.o`; output at `/tmp/kimicc_veryquick_after_func7fix.out` | The current broad run completes with `rc=1`: 52 errors out of 392295 tests. Known harness/platform-noise clusters include Tcl 8.5 `binary decode` failures (`fts5contentless3`, `fts3corrupt4`) and `Inf` versus `inf` casing (`func`, `json101`, `json501`, `literal`). Remaining likely kimicc-owned clusters to classify standalone against clang are `incrcorrupt`, `types3`, and `corruptI`. |
