...
| Fixed | C11 static assertion declarations | `moon test parser/parser_test.mbt --target native --filter 'parse static assertions at declaration scopes'`; `moon test test/e2e/e2e_test.mbt --target native --filter '_Static_assert declarations do not emit calls'` | xxHash's normal portability path expands static checks to `_Static_assert(...)` declarations inside `do { ... } while (0)` blocks. kimicc previously treated these as ordinary function calls and left an unresolved `__Static_assert` symbol. The parser now consumes `_Static_assert`, `__Static_assert`, and `static_assert` declarations at file, struct, and block scope as compile-time no-ops after checking constant-false assertions. |
| Fixed | Builtin 128-bit aliases in block declarations | `moon test parser/parser_test.mbt --target native --filter 'parse builtin 128-bit declarations in block scope'`; `moon test test/e2e/e2e_test.mbt --target native --filter 'builtin 128-bit aliases declare block locals'` | xxHash's scalar XXH3 path declares `__uint128_t const product` inside `XXH_mult64to128`. The parser now treats builtin 128-bit aliases as valid declaration starts in block and `for` initializer scopes. |
