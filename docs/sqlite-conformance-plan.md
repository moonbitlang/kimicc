...
| Open | Public SQLite Tcl `select1.test` floating-point runtime | `./testfixture /tmp/kimicc_sqlite_src_3049001/sqlite-src-3049001/test/select1.test` after linking `TESTFIXTURE_SRC1=/tmp/kimicc_sqlite3_testfixture.o` | The test no longer crashes and no longer fails with `database is locked`. It now reaches `select1-1.9` and reports floating-point mismatches such as `1.1` becoming `1.0`, `2.2` becoming `2.0`, `44.0` becoming `0.0`, and one `123.45` case becoming `100.0`; later it aborts with `unable to open database file` around line 1032. The clang-built baseline passes the same script, so the next failure class should focus on real/double value preservation in SQLite value storage, comparison, or result formatting before treating the late open error as independent. |
