...
- For broad `veryquick`/`full` runner failures, append `testrunner.tcl errors`
  output from the same work directory so the MoonBit comparison includes exact
  residual test IDs plus expected/got lines, not only the volatile parallel
  runner progress summary.
...
- Run those same 6 residual scripts against a copied, platform-adjusted public
  SQLite source tree where only the known Tcl/platform expectations are
  patched, then require kimicc-built, clang-built, and direct
  Mach-O-object-linked `testfixture` binaries to pass. The same adjusted root
  also passes a focused `testrunner.tcl --jobs 1 full json101` probe in all
  three modes.
- Run SQLite's whole public `full` Tcl permutation against that same
  platform-adjusted source copy, requiring kimicc-built, clang-built, and direct
  Mach-O-object-linked `testfixture` binaries to exit cleanly with no
  `FAILED:` runner markers.
...
KIMICC_SQLITE_CONFORMANCE=platform \
  moon test test/e2e/sqlite_test.mbt --target native \
  --filter 'sqlite public platform-adjusted residual scripts pass with clang and direct when enabled'

KIMICC_SQLITE_CONFORMANCE=platform-full \
  moon test test/e2e/sqlite_test.mbt --target native \
  --filter 'sqlite public platform-adjusted full suite passes with clang and direct when enabled'
...
2. External compiler testbed gate:
   ...
   modes with scalar XXH3 enabled, cJSON 1.7.18
   parse/edit/print/delete behavior in kimicc assembly and direct Mach-O object
...
5. SQLite platform-adjusted residual proof:
   ...
   This copies the public SQLite source tree to a temporary root and patches
   only the known Tcl/platform expectation drift. It should pass with kimicc,
   clang, and direct object mode; it does not replace testing the unmodified
   public source tree.

   A slower whole-suite variant is available when checking whether the adjusted
   public `full` permutation is green end-to-end:
...
7. Publish checklist:
   ...
   - Publish `cfront` before `kimicc`. The workspace resolves
     `bobzhang/cfront` locally, so a `kimicc` published against an
     unpublished or stale `cfront` version would have a dependency nobody
     else can resolve. Bump `cfront/moon.mod` and the matching pin in the
     root `moon.mod` together.
...
- [x] Add an opt-in release gate for previously fixed high-value public SQLite
  scripts.
- [x] Add an opt-in platform-adjusted residual gate proving the current
  unmodified residual set disappears when only known Tcl/platform expectations
  are patched in a temporary public-source copy.
- [x] Add an opt-in platform-adjusted `full` runner gate for the same temporary
  public-source copy.
...
| Passed | Deterministic scalar/control/aggregate differential corpora | `moon test test/e2e/confidence_test.mbt --target native --filter 'e2e deterministic scalar aggregate differential corpus matches clang'`; `moon test test/e2e/confidence_test.mbt --target native --filter 'e2e deterministic scalar control differential corpus matches clang'`; direct Mach-O variants in the same file | The normal confidence suite now generates small C programs, compiles each with both clang and kimicc, and compares exit code plus stdout. The corpora mix integer conversions, shifts, unsigned comparisons, struct initializers, small struct returns, aggregate arguments, loop-carried control flow, switch dispatch, and unsigned wraparound. Direct Mach-O variants run representative generated cases through the object writer and link/run them against the clang baseline. |
| Passed | Deterministic floating differential corpus | `moon test test/e2e/confidence_test.mbt --target native --filter 'e2e deterministic floating differential corpus matches clang'`; `moon test test/e2e/confidence_test.mbt --target native --filter 'e2e direct Mach-O floating differential corpus matches clang'` | The normal confidence suite now generates floating-point C programs that combine `double`, `float`, struct return/pass-by-value, comparisons, casts, loops, and integer-scaled output. The assembly path runs 8 clang differentials and the direct Mach-O path runs 4 representative differentials, covering FP register ABI and object emission in a cheap default gate. |
| Passed | Deterministic memory/pointer differential corpus | `moon test test/e2e/confidence_test.mbt --target native --filter 'e2e deterministic memory differential corpus matches clang'`; `moon test test/e2e/confidence_test.mbt --target native --filter 'e2e direct Mach-O memory differential corpus matches clang'` | The normal confidence suite now generates memory-mutating C programs that combine arrays of structs, pointer offsets, pointer subtraction, byte-buffer mutation, 2D array decay through `int *`, and derived-pointer stores. The assembly path runs 8 clang differentials and the direct Mach-O path runs 4 representative differentials, extending cheap default coverage for aliasing and address-generation behavior. |
| Passed | Deterministic variadic ABI differential corpus | `moon test test/e2e/confidence_test.mbt --target native --filter 'e2e deterministic variadic differential corpus matches clang'`; `moon test test/e2e/confidence_test.mbt --target native --filter 'e2e direct Mach-O variadic differential corpus matches clang'` | The normal confidence suite now generates mixed variadic C programs using `__builtin_va_list`, `__builtin_va_start`, and `__builtin_va_arg` for signed `int`, unsigned `int`, `long long`, and `double` unnamed arguments. The assembly path runs 8 clang differentials and the direct Mach-O path runs 4 representative differentials, guarding Darwin ARM64 vararg stack layout and typed `va_arg` widening. |
| Passed | Deterministic bit-field differential corpus | `moon test test/e2e/confidence_test.mbt --target native --filter 'e2e deterministic bitfield differential corpus matches clang'`; `moon test test/e2e/confidence_test.mbt --target native --filter 'e2e direct Mach-O bitfield differential corpus matches clang'` | The normal confidence suite now generates bit-field C programs that combine signed and unsigned fields, adjacent fields spanning a 32-bit allocation unit, nested struct layout, `sizeof` layout probes, repeated field updates, and signed field reads. The assembly path runs 8 clang differentials and the direct Mach-O path runs 4 representative differentials, extending cheap default coverage for bit-field layout and update semantics. |
...
| Passed | zlib compression/decompression smoke | `KIMICC_EXTERNAL_TESTBED=zlib moon test test/e2e/external_testbeds_test.mbt --target native` | The opt-in MoonBit e2e test rebuilds zlib 1.3.1 core sources from `/tmp/kimicc_zlib/zlib-1.3.1` through kimicc assembly and direct Mach-O object mode. Both modes link a kimicc-built smoke driver that uses `compress2`, `uncompress`, `zlibVersion`, and byte-for-byte output comparison, then exits with status 42. Routine tests skip this unless `KIMICC_EXTERNAL_TESTBED=zlib` or `all` is set. |
| Fixed | Clang rotate-left/right builtins | `moon test test/e2e/e2e_test.mbt --target native --filter '__builtin_rotateleft and rotateright'`; `moon test test/e2e/macho_object_test.mbt --target native --filter 'direct macho object supports bit count and byte swap builtins'` | xxHash exposed `__builtin_rotateleft32` and `__builtin_rotateleft64` as unresolved external calls once Clang builtin suppression was removed. kimicc now lowers 32-bit and 64-bit rotate-left/right builtins to AArch64 variable shifts and `orr`, and the direct Mach-O path encodes the same instructions. |
| Fixed | C11 static assertion declarations | `moon test cfront/parser/parser_test.mbt --target native --filter 'parse static assertions at declaration scopes'`; `moon test test/e2e/e2e_test.mbt --target native --filter '_Static_assert declarations do not emit calls'` | xxHash's normal portability path expands static checks to `_Static_assert(...)` declarations inside `do { ... } while (0)` blocks. kimicc previously treated these as ordinary function calls and left an unresolved `__Static_assert` symbol. The parser now consumes `_Static_assert`, `__Static_assert`, and `static_assert` declarations at file, struct, and block scope as compile-time no-ops after checking constant-false assertions. |
| Fixed | Builtin 128-bit aliases in block declarations | `moon test cfront/parser/parser_test.mbt --target native --filter 'parse builtin 128-bit declarations in block scope'`; `moon test test/e2e/e2e_test.mbt --target native --filter 'builtin 128-bit aliases declare block locals'` | xxHash's scalar XXH3 path declares `__uint128_t const product` inside `XXH_mult64to128`. The parser now treats builtin 128-bit aliases as valid declaration starts in block and `for` initializer scopes. |
| Fixed | `__builtin_prefetch` hint lowering | `moon test test/e2e/e2e_test.mbt --target native --filter '__builtin_prefetch is a no-op hint'`; `moon test test/e2e/macho_object_test.mbt --target native --filter 'direct macho object supports bit count and byte swap builtins'` | xxHash calls `__builtin_prefetch` in update loops. kimicc now lowers it as a side-effect-free hint, avoiding unresolved external symbols in both assembly and direct Mach-O object modes. |
| Fixed | Aligned array member pointer arithmetic | `moon test test/e2e/e2e_test.mbt --target native --filter 'aligned struct array member pointer arithmetic keeps full address'`; `moon test test/e2e/macho_object_test.mbt --target native --filter 'direct macho object keeps aligned struct array pointer arithmetic address'` | xxHash's `state->buffer + state->bufferedSize` uses an `_Alignas(64)` array member. The binary emitter now peels alignment wrappers before pointer/array arithmetic detection, preserving the full 64-bit address instead of narrowing the result to `int`. |
| Fixed | Aligned global array initializers | `moon test test/e2e/e2e_test.mbt --target native --filter 'aligned global byte array keeps initializer'`; `moon test test/e2e/macho_object_test.mbt --target native --filter 'direct macho object keeps aligned global byte array initializer'` | xxHash's `_Alignas(64) static const unsigned char XXH3_kSecret[192]` was emitted as zero-filled storage because global initializer emission did not unwrap aligned aggregate types. Global data emission now unwraps `Aligned(...)` for scalar/array initializers and honors stronger global alignment. |
| Passed | xxHash checksum/streaming smoke | `KIMICC_EXTERNAL_TESTBED=xxhash moon test test/e2e/external_testbeds_test.mbt --target native` | The opt-in MoonBit e2e test rebuilds xxHash 0.8.2 from `/tmp/kimicc_xxhash/xxHash-0.8.2` through kimicc assembly and direct Mach-O object mode using scalar XXH3 (`-DXXH_VECTOR=0 -U__ARM_NEON -U__ARM_NEON__`) and the normal C11 static assertion path. Both modes link a kimicc-built smoke driver that checks known XXH32, XXH64, XXH3-64, seeded XXH3-64, and XXH3-128 constants, and verifies XXH64 and XXH3 streaming APIs match their one-shot hashes. The full NEON/SIMD header path remains out of scope for this fixture profile. Routine tests skip this unless `KIMICC_EXTERNAL_TESTBED=xxhash` or `all` is set. |
| Passed | cJSON parse/edit/print smoke | `KIMICC_EXTERNAL_TESTBED=cjson moon test test/e2e/external_testbeds_test.mbt --target native` | The opt-in MoonBit e2e test rebuilds cJSON 1.7.18 from `/tmp/kimicc_cjson/cJSON-1.7.18` through kimicc assembly and direct Mach-O object mode. Both modes link a kimicc-built smoke driver that parses nested JSON, checks strings, arrays, booleans, and doubles, mutates the object, prints it, frees the printed buffer, and deletes the tree. Routine tests skip this unless `KIMICC_EXTERNAL_TESTBED=cjson` or `all` is set. |
| Passed | inih callback parser smoke | `KIMICC_EXTERNAL_TESTBED=inih moon test test/e2e/external_testbeds_test.mbt --target native` | The opt-in MoonBit e2e test rebuilds inih r58 from `/tmp/kimicc_inih/inih-r58` through kimicc assembly and direct Mach-O object mode. Both modes link a kimicc-built smoke driver that enables line-number callbacks, section callbacks, no-value entries, and multiline values, then verifies callback counts, parsed fields, and decimal conversion. Routine tests skip this unless `KIMICC_EXTERNAL_TESTBED=inih` or `all` is set. |
...
| Passed | SQLite platform-adjusted residual scripts | `KIMICC_SQLITE_CONFORMANCE=platform moon test test/e2e/sqlite_test.mbt --target native --filter 'sqlite public platform-adjusted residual scripts pass with clang and direct when enabled'` | The opt-in gate copies the public SQLite source tree to `/tmp/kimicc_sqlite_src_3049001_platform`, patches only the known Tcl/platform expectation drift in `func`, `json101`, `json501`, `literal`, `types3`, and `loadext`, then requires those formerly residual scripts to pass with kimicc-built, clang-built, and direct Mach-O-object-linked `testfixture` binaries. It also runs a focused adjusted `testrunner.tcl --jobs 1 full json101` probe in all three modes. The original public source tree remains unmodified and covered by the residual-signature guards. |
| Passed | SQLite platform-adjusted full suite | `KIMICC_SQLITE_CONFORMANCE=platform-full moon test test/e2e/sqlite_test.mbt --target native --filter 'sqlite public platform-adjusted full suite passes with clang and direct when enabled'` | The opt-in gate runs SQLite's whole public `full` permutation against the same temporary adjusted source root and passed with kimicc-built, clang-built, and direct Mach-O-object-linked `testfixture` binaries. All three modes exited cleanly with no `FAILED:` runner markers. This strengthens the current analysis that the unmodified public `full` residuals are platform/Tcl expectation drift, not a known kimicc-only SQLite failure. |
