...
| Open | Public SQLite Tcl `corruptI.test` residual failure | `./testfixture /tmp/kimicc_sqlite_src_3049001/sqlite-src-3049001/test/corruptI.test`; same command with `testfixture-clang` | The kimicc-built fixture still fails `corruptI-6.1` with `database disk image is malformed`, while the clang-built fixture passes 0 errors out of 24 tests. This is the next kimicc-owned SQLite cluster to minimize. |
| Open | SQLite `veryquick.test` post-`incrcorrupt` frontier | `./testfixture /tmp/kimicc_sqlite_src_3049001/sqlite-src-3049001/test/veryquick.test` after relinking `/tmp/kimicc_sqlite3_testfixture.o`; output at `/tmp/kimicc_veryquick_after_dbfileversfix.out` | The current broad run completes with `rc=1`: 39 errors out of 392295 tests. The previous `incrcorrupt-*` failures are gone. Known harness/platform-noise clusters include Tcl 8.5 `binary decode` failures (`fts5contentless3`, `fts3corrupt4`), `Inf` versus `inf` casing (`func`, `json101`, `json501`, `literal`), and the clang-matching `types3-1.1` Tcl expectation issue. The remaining kimicc-owned cluster in this broad list is `corruptI-6.1`. |
