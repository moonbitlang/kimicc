...
| Open | QuickJS reduced embed smoke after compound-literal fix | `/tmp/kimicc_quickjs_smoke/run_emscripten_noatomics/embed_smoke`, built from QuickJS 2025-09-13 with atomics/stack-check disabled and support files compiled by clang | The reduced smoke now preprocesses, compiles `quickjs.c` with kimicc, assembles, and links. It no longer crashes in `JS_NewRuntime2`; the next runtime frontier aborts while freeing a value during `JS_AddIntrinsicBasicObjects`. Normal macOS QuickJS preprocessing also still leaves compiler builtins such as fortified `snprintf`, C11 atomics, and `__builtin_frame_address`, so full unmodified QuickJS needs either builtin lowering or a sanitized preprocessor profile. |
