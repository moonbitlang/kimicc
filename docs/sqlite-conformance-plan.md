...
| Open | Public SQLite Tcl `expert1.test` after pointer comparison fix | `./testfixture /tmp/kimicc_sqlite_src_3049001/sqlite-src-3049001/ext/expert/expert1.test` after linking `TESTFIXTURE_SRC1=/tmp/kimicc_sqlite3_testfixture.o` | The next `veryquick.test` blocker is a crash/SIGBUS after `expert1-2.19.0` (`rc=138`). The clang-linked baseline passes `expert1.test` with 0 errors out of 75 tests. This is the next failure class to localize. |
