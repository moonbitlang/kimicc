...
| Open | SQLite `avtrans.test` database corruption | `./testfixture /tmp/kimicc_sqlite_src_3049001/sqlite-src-3049001/test/avtrans.test` from an isolated temporary working directory after linking `TESTFIXTURE_SRC1=/tmp/kimicc_sqlite3_testfixture.o` | After the RTree and float-suffix fixes, `veryquick.test` exits normally but fails in `avtrans.test`. The kimicc fixture fails `avtrans-9.2.1-1024` and `avtrans-9.2.2-1024` with row-count/hash mismatches (`1024 46d27034...` expected, `439 a81bc...` and `81 5b042...` observed), then reports `database disk image is malformed`. The clang-linked testfixture passes standalone `avtrans.test` with 0 errors out of 315 tests. Next step is to minimize `avtrans` around page-size 1024 autovacuum/fullfsync transaction behavior and identify whether corruption begins in pager, btree, or VFS-facing codegen. |
