...
| Open | SQLite recover extension crash | `./testfixture /tmp/kimicc_sqlite_src_3049001/sqlite-src-3049001/ext/recover/recover1.test` | The next `veryquick.test` run gets past FTS5 and `intck`, then the kimicc-built fixture crashes with macOS `Bus error: 10` at `recover1-16.1` (`rc=138`). The matching clang-built fixture passes `recover1.test` with 0 errors out of 105 tests. Next step is to isolate `recover1-16.1`, compare the recovery API state under clang and kimicc, and minimize the compiler bug behind the crash. |
