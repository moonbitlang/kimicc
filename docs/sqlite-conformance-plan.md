...
2. External compiler testbed gate:
   ...
   direct Mach-O object modes, xxHash 0.8.2 checksum and streaming APIs in
   kimicc assembly and direct Mach-O object modes, cJSON 1.7.18
...
| Passed | xxHash checksum/streaming smoke | `KIMICC_EXTERNAL_TESTBED=xxhash moon test test/e2e/external_testbeds_test.mbt --target native` | The opt-in MoonBit e2e test rebuilds xxHash 0.8.2 from `/tmp/kimicc_xxhash/xxHash-0.8.2` through kimicc assembly and direct Mach-O object mode using the normal C11 static assertion path. Both modes link a kimicc-built smoke driver that checks known XXH32 and XXH64 constants and verifies the XXH64 streaming API matches the one-shot hash. Routine tests skip this unless `KIMICC_EXTERNAL_TESTBED=xxhash` or `all` is set. |
