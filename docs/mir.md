...
- `Program::interpret_i64` is an integer-only interpreter intended for
  ...
  `%s` and nonnegative decimal `%d`/`%i`/`%u`, or escaped `%%`.
  ...
  simple numeric precision for literal-string `%s` and nonnegative decimal
  `%d`/`%i`/`%u`, or escaped `%%`; output contents are not modeled.
