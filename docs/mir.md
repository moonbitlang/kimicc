...
- `Program::interpret_i64` is an integer-only interpreter intended for
  ...
  `%s` and nonnegative integer `%d`/`%i`/`%u`/`%o`/`%x`/`%X`, or escaped `%%`.
  ...
  simple numeric precision for literal-string `%s` and nonnegative integer
  `%d`/`%i`/`%u`/`%o`/`%x`/`%X`, or escaped `%%`; output contents are not modeled.
