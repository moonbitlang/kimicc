...
- `Program::interpret_body_i64` executes those lowered MIR bodies directly. It
  ...
  aggregate fields, `break`/`continue`, and ternaries. Missing bodies return
  `Err` instead of falling back to parser AST execution.
- `codegen/mir_body_codegen.mbt` is the first production backend consumer of
  ...
  assignment, local postfix updates, scalar `*&` / `&*` cancellation,
  simple scalar local pointer loads/stores, scalar local arrays, direct scalar
  aggregate fields, and arithmetic/logical operators. Unsupported MIR bodies
