...
- The preprocessor seeds Linux/x86-64 target macros such as `__linux__`,
  ...
  `__SIZEOF_*`/width/max-value macros, floating-point limit macros used by
  Clang's `<float.h>`, atomic memory-order constants, and ELF empty
...
- `_Alignas` and numeric GNU `__attribute__((aligned(N)))`/`__aligned__(N)`
  ...
  Functions with locals requiring more than 16-byte alignment keep `rbp` as the
  incoming frame anchor and use an aligned local-frame base, preserving the
  callee-saved register used for that base before returning.
